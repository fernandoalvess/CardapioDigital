...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  if (!isSupabaseConfigured) {
    return NextResponse.json({ error: "Conecte o Supabase para gerenciar comandas." }, { status: 503 });
  }

  const { id } = await params;
  const parsed = actionSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "Dados inválidos para atualizar a comanda." }, { status: 400 });
  }
  ...
  if (parsed.data.action === "close") {
    const { error } = await supabase.rpc("close_order_comanda", { target_order_id: id });
    return error
  ...
  if (parsed.data.action === "cancel") {
    const { error } = await supabase.rpc("cancel_order_comanda", { target_order_id: id });
    return error
      ? NextResponse.json({ error: cleanError(error.message) }, { status: 400 })
      : NextResponse.json({ ok: true });
  }
